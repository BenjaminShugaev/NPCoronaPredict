...
constexpr int steps = 512; // this is the resolution along the z-axis used for integration. There is no reason to modify this; the default value of 512 produces a resolution of 0.004 nm and the input potentials are less precise than this (or just
                           // thermal fluctuations at this resolution)
...
bool
CheckForPrecalculated (const double *adsorption_energy, const double *adsorption_error, const double *mfpt_val, const double *minloc_val, const double radius, const double zeta, const std::string &name, const std::string &directory,
                       const std::string &npName, int isMFPT = 0, int appendAngle = 0, double omegaAngle = 0)
{
  std::string filename;
  ...
  if (isMFPT == 1)
    {
      filename = directory + "/" + npName + "/" + TargetList::Filename (name) + "_" + std::to_string (static_cast<int> (radius)) + "_" + std::to_string (static_cast<int> (1000 * zeta)) + cylinderFileNameAppend + "_mfpt.uam";
    }
  else
    {
      filename = directory + "/" + npName + "/" + TargetList::Filename (name) + "_" + std::to_string (static_cast<int> (radius)) + "_" + std::to_string (static_cast<int> (1000 * zeta)) + cylinderFileNameAppend + ".uam";
    }
...
std::string
PrecalcName (const double *adsorption_energy, const double *adsorption_error, const double *mfpt_val, const double *minloc_val, const double radius, const double zeta, const std::string &name, const std::string &directory,
                       const std::string &npName, int isMFPT = 0, int appendAngle = 0, double omegaAngle = 0)
{
  std::string filename;
  std::string cylinderFileNameAppend;
  ...
  for (int i = 0; i < iterations; ++i)
    {
      ...
      handle << std::left << std::setw (14) << std::fixed << std::setprecision (5) << protein_area[i];
...
void
PrintStatistics (const double *adsorption_energy, const double *adsorption_error, const double radius, const std::string &filename)
{
  double sin_theta;
  double error = 0.0;
...
inline void
Rotate (const int size, const double phi, const double theta, const std::vector<double> &cx, const std::vector<double> &cy, const std::vector<double> &cz, double *x, double *y, double *z)
{
  double R[3][3];
...
inline void
Rotate3 (const int size, const double phi, const double theta, const double omega, const std::vector<double> &cx, const std::vector<double> &cy, const std::vector<double> &cz, double *x, double *y, double *z)
{
  double R[3][3];
...
inline void
JitterPDB (const int size, const double jitterMagnitude, double *x, double *y, double *z)
{
  /*
      R[0][0] = std::cos(theta) * std::cos(phi) * std::cos(omega) - std::sin(omega) * std::sin(phi);
      R[0][1] = -1.0 * std::cos(theta) * std::sin(phi) * std::cos(omega) - std::cos(phi)*std::sin(omega);
...
inline void
RotateZ (const int size, const double omega, const std::vector<double> &cx, const std::vector<double> &cy, const std::vector<double> &cz, double *x, double *y, double *z)
{
  double R[3][3];
...
void
SquareXY (const int size, double *x, double *y)
{
  for (int i = 0; i < size; ++i)
    {
      x[i] *= x[i];
...
void
ShiftZ (const int size, double *z)
{
...
void
Sum (const int size, double *result, double *arr)
{
  *result = 0.0;
  for (int i = 0; i < size; ++i)
    {
...
void
Integrate (const int size, const double dz, const double init_energy, const double *energy, const double *ssd, double *adsorption, double temperature)
{
...
void
IntegrateMFPT (const int size, const double dz, const double init_energy, const double *energy, const double *ssd, double *mfpt, int npType, int calculateMFPT, double temperature)
{
  long double res = 0.0;
      ...
      for (int k = std::max (minI - minimumRange * stepSize, 0); k < std::min (size, minI + minimumRange * stepSize + 1); k += stepSize)
        {
          ...
          res = 0.0;
          for (int i = 0; i < k; ++i)
            { // integrate over "y", runs from r0 to the escape point
              // here ssd[i] = y and ssd[j] = rprime
              innerRes = 0;
              for (int j = size; j > i; --j)
                { // integrate over "x": from touching the NP to the  value of y
...
void
IntegrateCylinder (const int size, const double dz, const double init_energy, const double *energy, const double *ssd, double *adsorption, double temperature)
{
  long double area = 0.0;
  for (int i = 0; i < size; ++i)
    {
...
void
IntegrateCube (const int size, const double dz, const double init_energy, const double *energy, const double *ssd, double *adsorption, double temperature)
{
  long double area = 0.0;
  for (int i = 0; i < size; ++i)
    {
...
void
MeanAndSD (const int size, double *mean, double *sd, double *arr)
{
  double lmean = 0;
  double lsd = 0;
  for (int i = 0; i < size; ++i)
...
void
BoltzMeanAndSD (const int size, double *mean, double *sd, double *arr)
{
  double lmeanN = 0;
...
double
getDistance (double x, double y, double z, double start, double radius, int npType)
{
  double distance;
...
double
getDistance3D (double aaX, double aaY, double aaZ, double npX, double npY, double npZ, double npRadius, int npType)
{
  double distance = 0;
...
// returns the total energy for the biomolecule with the set of NP beads
double
GetEnergy (double *x, double *y, double *z, int j, double dr, bool sumNPs, int numNPs, int size, const PDB &pdb, const NP &np, const Potentials &potentials, double radius, int npType)
{
...
double
GetInternalEnergy (double *x, double *y, double *z, int j, double dr, bool sumNPs, int numNPs, int size, const PDB &pdb, const NP &np, const Potentials &potentials, double radius, int npType)
{
  double totalEnergy = 0;
  return totalEnergy;
}
...
double
GetPotGrad (double *x, double *y, double *z, int j, double dr, bool sumNPs, int numNPs, int dir, const PDB &pdb, const NP &np, const Potentials &potentials, double radius, int npType)
{
  double dudx = 0;
...
double
GetPotGrad4 (double *x, double *y, double *z, int j, double dr, bool sumNPs, int numNPs, int dir, const PDB &pdb, const NP &np, const Potentials &potentials, double radius, int npType)
{
  double dudx = 0;
  double dx = (dir == 0) ? dr / 2 : 0.0;
  double dy = (dir == 1) ? dr / 2 : 0.0;
...
double
GetPotGrad4TotalPoint (double x, double y, double z, int j, double dr, bool sumNPs, int numNPs, int dir, const PDB &pdb, const NP &np, const Potentials &potentials, double radius, int npType)
{
  double dudx = 0;
  double dx = (dir == 0) ? dr / 2 : 0.0;
  double dy = (dir == 1) ? dr / 2 : 0.0;
  ...
  for (int k = 0; k < numNPs; ++k)
    {
      ...
      if (sumNPs == false)
        {
          dxp2 = getDistance3D (x + 2 * dx, y + 2 * dy, z + 2 * dz, np.m_x[k], np.m_y[k], np.m_z[k], np.m_radius[np.m_npBeadType[k]], np.m_shape[np.m_npBeadType[k]]);
          dxp1 = getDistance3D (x + dx, y + dy, z + dz, np.m_x[k], np.m_y[k], np.m_z[k], np.m_radius[np.m_npBeadType[k]], np.m_shape[np.m_npBeadType[k]]);
          dxm1 = getDistance3D (x - dx, y - dy, z - dz, np.m_x[k], np.m_y[k], np.m_z[k], np.m_radius[np.m_npBeadType[k]], np.m_shape[np.m_npBeadType[k]]);
          dxm2 = getDistance3D (x - 2 * dx, y - 2 * dy, z - 2 * dz, np.m_x[k], np.m_y[k], np.m_z[k], np.m_radius[np.m_npBeadType[k]], np.m_shape[np.m_npBeadType[k]]);
          ep2 = static_cast<double> (potentials[pdb.m_id[j]].Value (dxp2, np.m_npBeadType[k]));
          ep1 = static_cast<double> (potentials[pdb.m_id[j]].Value (dxp1, np.m_npBeadType[k]));
          em1 = static_cast<double> (potentials[pdb.m_id[j]].Value (dxm1, np.m_npBeadType[k]));
          em2 = static_cast<double> (potentials[pdb.m_id[j]].Value (dxm2, np.m_npBeadType[k]));
        ...
        {
          dxp2 = getDistance3D (x + 2 * dx, y + 2 * dy, z + 2 * dz, 0, 0, 0, radius, npType);
          dxp1 = getDistance3D (x + dx, y + dy, z + dz, 0, 0, 0, radius, npType);
          dxm1 = getDistance3D (x - dx, y - dy, z - dz, 0, 0, 0, radius, npType);
          dxm1 = getDistance3D (x - 2 * dx, y - 2 * dy, z - 2 * dz, 0, 0, 0, radius, npType);
...
inline double
thetaClip (double theta1, double theta2)
{
  double dtheta = theta1 - theta2;
  if (dtheta > 0)
    {
      return theta2 + std::min (dtheta, 0.5);
    }
  else
    {
      return theta2 + std::max (dtheta, -0.5);
    }
}
...
inline void
RelaxPDB (const int size, double *x, double *y, double *z, const Config &config, const Potentials &potentials, const PDB &pdb, const NP &np, double radius, int npType, bool applyFinalShift)
{
  // int numSteps = 10000;
  ...
  */
  ...
  bool doCentralPull = config.m_pullIsCentral; // if false, pull is in -z only
  ...
  double kbTVal = 1.0;
  double dtValInit = 1e-11;
  double dtVal = dtValInit;
  double dtNew = dtVal;

  // GJF alpha is equal to gamma*mass

  double nominalDiffusionCoeff = 1e9; // kbT/(gamma * mass)
  ...
  */
  double maxDisplacement = 0.05; // set the maximum a bead can displace in a single step (in nm) - this is use to define the first step size for the BB method
  // note that this implies updateScaleSize*force should ideally be less than  this value
  ...
  // update this array with the current optimium positions - these are used in case the rigid solver gets stuck and needs to rset
  for (int j = 0; j < size; ++j)
    {
      xFin[j] = x[j];
      yFin[j] = y[j];
  ...
  for (int s = 0; s < numRigidSteps; ++s)
    {
      ...
      for (int j = 0; j < size; ++j)
        {
          ...
          double forceClipVal = 1000.0;
          fxj = std::max (-forceClipVal, std::min (fxj, forceClipVal));
          fyj = std::max (-forceClipVal, std::min (fyj, forceClipVal));
          fzj = std::max (-forceClipVal, std::min (fzj, forceClipVal));
          ...
          */
          if (std::fabs (fxj) > 1e5 || std::fabs (fyj) > 1e5 || std::fabs (fzj) > 1e5)
            {
              // rigidDT = rigidDT*0.1;
              // continue;
              std::cout << "rigid optimisation requires an extreme force, quitting \n";
              rigidDone = true;
            }
          ...
          fcomy += fyj;
          fcomz += fzj;
          double rloc = sqrt (x[j] * x[j] + y[j] * y[j] + z[j] * z[j]);

          /*
        if( std::fabs(fxj) > 100 ){
        std::cout << " atom " << j << " at " << x[j]<<","<<y[j]<<","<<z[j]<<":"<<rloc   <<  " produces large x force " << fxj <<"\n";
        }

        if( std::fabs(fyj) > 100 ){
        std::cout << " atom " << j << " at " << x[j]<<","<<y[j]<<","<<z[j]<<":"<<rloc <<  " produces large x force " << fyj <<"\n";
        }
        if( std::fabs(fzj) > 100 ){
        std::cout << " atom " << j << " at " << x[j]<<","<<y[j]<<","<<z[j]<<":"<<rloc <<  " produces large z force " << fzj <<"\n";
        }
        */
      ...
      // double maxDisplaceX = std::fabs( taux*10 );
      // double maxDisplaceX = std::fabs( tauy*10 );
      // double maxDisplaceX = std::fabs( tauz*10 );
      ...
      for (int j = 0; j < size; ++j)
        {
          ...
          double axj = ((-(ixz * iyy * taux) + ixy * iyz * taux + ixy * ixz * tauy - ixx * iyz * tauy - std::pow (ixy, 2) * tauz + ixx * iyy * tauz) * yrel
                        + (ixy * izz * taux + std::pow (ixz, 2) * tauy - ixx * izz * tauy + ixx * iyz * tauz - ixz * (iyz * taux + ixy * tauz)) * zrel)
                       / inertiaDenom;

          double ayj = ((ixz * iyy * taux - ixy * iyz * taux - ixy * ixz * tauy + ixx * iyz * tauy + std::pow (ixy, 2) * tauz - ixx * iyy * tauz) * xrel
                        + (-(std::pow (iyz, 2) * taux) + iyy * izz * taux + ixz * iyz * tauy - ixy * izz * tauy - ixz * iyy * tauz + ixy * iyz * tauz) * zrel)
                       / inertiaDenom;

          double azj = ((ixz * iyz * taux - ixy * izz * taux - std::pow (ixz, 2) * tauy + ixx * izz * tauy + ixy * ixz * tauz - ixx * iyz * tauz) * xrel
                        + (std::pow (iyz, 2) * taux - iyy * izz * taux + ixy * izz * tauy + ixz * iyy * tauz - iyz * (ixz * tauy + ixy * tauz)) * yrel)
                       / inertiaDenom;
          ...
          //if(j==indexAtom){
              std::cout << "broke with denominator: " << inertiaDenom << "\n";
              ...
              std::cout << "forces: " << taux << " " << tauy << " " << tauz << "\n";
              std::cout <<"acceleration: "<< axj << "  " << ayj << " " << azj << "\n";
              //std::cout << "old acceleration  " << axjold  << " " << ayjold << " " << azjold<< "\n";
      ...
      // post-step check
      double finalZOffset = -1.0;
      for (int j = 0; j < size; ++j)
        {
          // std::cout << "checking atom " << j <<  "R " <<sqrt(x[j]+x[j] + y[j]*y[j] + z[j]*z[j] ) << "\n";
          if (npType == 1)
            {
              double safetyRadius = radius + 0.01;
              double surfaceDist = sqrt (x[j] + x[j] + y[j] * y[j] + z[j] * z[j]) - safetyRadius;
              if (surfaceDist < 0)
                {
                  finalZOffset = std::max (finalZOffset, sqrt (safetyRadius * safetyRadius - x[j] * x[j] - y[j] * y[j]) - z[j]);
                }
            }
        }
      if (finalZOffset > 0)
        {
          // std::cout << "rigid dynamics: biomolecule is inside NP. ejecting and reducing timestep \n";
          for (int j = 0; j < size; ++j)
            {

              z[j] += finalZOffset;
            }
          rigidDT = rigidDT * 0.5; // decrease the timestep
        }

      // if(j==indexAtom){

      // check energies at the current state, saving the best
      double currentTotalEnergy = GetEnergy (x, y, z, 0, dr, config.m_sumNPPotentials, numNPs, size, pdb, np, potentials, radius, npType);

      if (currentTotalEnergy < bestRigidEnergy)
        {
          for (int j = 0; j < size; ++j)
            {
              xFin[j] = x[j];
          ...
          bestRigidEnergy = currentTotalEnergy;
        }
      ...
      if (currentTotalEnergy > bestRigidEnergy + 5.0)
        {
          // this likely means the biomolecule has become detached. restore to best and decrease timestep.
          rigidDT = rigidDT * 0.5;
          for (int j = 0; j < size; ++j)
            {
              x[j] = xFin[j];
      ...
      // energy check and restore if needed
      // double currentTotalEnergy = GetEnergy(x, y,z, 0,  dr, config.m_sumNPPotentials,  numNPs, size,  pdb,np, potentials, radius,npType);
    }

  // load in the lowest energy structure
  ...
  // std::vector<double> zReshape(size,0.0);
  // double alphaRho= 3.0 - sqrt(2.0);
  bool doProteinPrebend = true;
  bool reshapedFailed = false;
  // std::cout << npType << "\n";

  // vertical-shift prebending
  // compute a grid of allowed shifts in the z-direction that bring the closest resiude into contact with the NP

  double shiftGridRes = 0.5;                    // in nm - roughly covers one residue
  double targetRCurve = std::max (radius, 6.0); // typical radius of curvature for an alpha helix - we assume the protein can be bent no more than this

  double targetDropPerNM = 1.0 / 1.0;
  double shiftMaxGrad = targetDropPerNM * shiftGridRes; // two neighbouring cells can only differ by this amount
  ...
  int numShiftXPoints = 2 + (int)round ((xMax - xMin) / shiftGridRes);
  int numShiftYPoints = 2 + (int)round ((yMax - yMin) / shiftGridRes);
  // std::cout << " will prepare grid with " << numShiftXPoints <<" , " << numShiftYPoints << "\n";
  // std::vector<std::vector<double>>  shiftGrid( numShiftXPoints   , std::vector<double>(numShiftYPoints, 0.0) ) ; //       ]         [numShiftYPoints] ;

  double shiftGrid[numShiftXPoints][numShiftYPoints];
  // std::cout << "starting bending \n";
  // prepopulate the array with allowed shifts - this ensures the centre remains unshifted and places an upper bound on all other points
  for (int i = 0; i < numShiftXPoints; ++i)
    {
      for (int j = 0; j < numShiftYPoints; ++j)
        {

          double xVal = i * shiftGridRes + xMin;
          double yVal = j * shiftGridRes + yMin;
          double radialDist = sqrt (xVal * xVal + yVal * yVal);
          if (npType == 2 || npType == 4 || npType == 5)
            {
              radialDist = sqrt (yVal * yVal);
            }
          shiftGrid[i][j] = radialDist * shiftMaxGrad; //set a default value
          if(npType != 3){
            if (radialDist < targetRCurve)
              {
                shiftGrid[i][j] = targetRCurve - sqrt (targetRCurve * targetRCurve - radialDist * radialDist);
              }
            else
              {
                shiftGrid[i][j] = targetRCurve;
              }
          }
  ...
  double safeRadius = radius + 0.3; //make sure the beads aren't shifted too close to the surface
  if (doProteinPrebend == true)
    {
      for (int i = 0; i < size; ++i)
        {
          int xi = (int)round ((x[i] - xMin) / shiftGridRes);
          int yi = (int)round ((y[i] - yMin) / shiftGridRes);
          xi = std::max (std::min (xi, numShiftXPoints - 1), 0);
          yi = std::max (std::min (yi, numShiftYPoints - 1), 0);

          double distToNP = z[i]; 
          if (x[i] * x[i] + y[i] * y[i] < safeRadius * safeRadius && npType == 1)
            {
              // xi^2 + yi^2 + zs^2 == radius^2
              double sphereZPoint = sqrt (safeRadius * safeRadius - x[i] * x[i] - y[i] * y[i]);
              distToNP = z[i] - sphereZPoint;
            }
          if(npType == 3){
            if( x[i]<- radius || x[i]>radius || y[i] > radius || y[i] <-radius){
            //point falls outside the square, allow this to fall to the z=0 place
             distToNP = z[i];
            }
            else{
              //inside the projected NP top, clip to the surface
              distToNP  = z[i] - safeRadius; 
             }
          
         }


          if (y[i] * y[i] < safeRadius * safeRadius && (npType == 2 || npType == 4 || npType == 5))
            {
              // xi^2 + yi^2 + zs^2 == radius^2
              double sphereZPoint = sqrt (safeRadius * safeRadius - y[i] * y[i]);
              distToNP = z[i] - sphereZPoint;
            }

          shiftGrid[xi][yi] = std::min (distToNP, shiftGrid[xi][yi]);
        }

      // clipping - make sure the gradient in each orthogonal direction is kept under a limit
      int xi0 = (int)round ((0.0 - xMin) / shiftGridRes);
      int yj0 = (int)round ((0.0 - yMin) / shiftGridRes);
      // std::cout << "starting clipping \n";
      for (int yj = 0; yj < numShiftYPoints; ++yj)
        {
          for (int xi = xi0 + 1; xi < numShiftXPoints; ++xi)
            {
              shiftGrid[xi][yj] = std::min (shiftGrid[xi][yj], shiftGrid[xi - 1][yj] + shiftMaxGrad);
            }
          for (int xi = xi0 - 1; xi >= 0; --xi)
            {
              shiftGrid[xi][yj] = std::min (shiftGrid[xi][yj], shiftGrid[xi + 1][yj] + shiftMaxGrad);
            }
        }

      for (int xi = 0; xi < numShiftXPoints; ++xi)
        {
          for (int yj = yj0 + 1; yj < numShiftYPoints; ++yj)
            {
              shiftGrid[xi][yj] = std::min (shiftGrid[xi][yj], shiftGrid[xi][yj - 1] + shiftMaxGrad);
            }
          for (int yj = yj0 - 1; yj >= 0; --yj)
            {
              shiftGrid[xi][yj] = std::min (shiftGrid[xi][yj], shiftGrid[xi][yj + 1] + shiftMaxGrad);
            }
        }
      ...
      for (int i = 0; i < size; ++i)
        {
          ...
          int xi2 = xi - 1;
          int yi2 = yi - 1;
          if (xi < numShiftXPoints / 2.0)
            {
              xi2 = xi + 1;
            }
          if (yi < numShiftYPoints / 2.0)
            {
              yi2 = yi + 1;
            }

          // local interpolation
          double x1 = xi * shiftGridRes + xMin;
          double x2 = xi2 * shiftGridRes + xMin;
          double y1 = yi * shiftGridRes + yMin;
          double y2 = yi2 * shiftGridRes + yMin;
          double fxy1 = (x2 - x[i]) / (x2 - x1) * shiftGrid[xi][yi] + (x[i] - x1) / (x2 - x1) * shiftGrid[xi2][yi];
          double fxy2 = (x2 - x[i]) / (x2 - x1) * shiftGrid[xi][yi2] + (x[i] - x1) / (x2 - x1) * shiftGrid[xi2][yi2];
          double finalShift = (y2 - y[i]) / (y2 - y1) * fxy1 + (y[i] - y1) / (y2 - y1) * fxy2;

          // std::cout << x[i] << ":" << xi << " , " << y[i] << ":" << yi << "," << z[i] << " shifting by -" << shiftGrid[xi][yi] << "\n";
          z[i] = z[i] - finalShift ;
        }
    }


  //after shifting and rigid dynamics, re-minimise w.r.t z
   //second minimisation pass
  minEnergy = 200;
  minEnergyDZI = 0;


  int numDZIPoints2 = int( 0.5/initScanDZ);
  for (int dzi = -numDZIPoints2; dzi < numDZIPoints2; ++dzi)
    {
      double totalEnergy = 0;
      for (int i = 0; i < size; ++i)
        {
  ...
  // std::cout << "all shifts applied, segfault is somewhere following \n";
  ...
  double wcaEpsilon = 10; // kBT
  double wcaSigma = 0.4;  // nm
                          // writePDB("test_pre", "pdbouttest", size,  pdb, x, y, z);

  int numAverageStepsDone = 0;

  // double ez0 = GetEnergy(0, 0, radius +  1.00, 0,  dr, config.m_sumNPPotentials,  numNPs, size,  pdb,np, potentials, radius,npType);
  // double ez1 = GetEnergy(0, 0, radius +  1.05, 0,  dr, config.m_sumNPPotentials,  numNPs, size,  pdb,np, potentials, radius,npType);
  // double gradFromPotential = -(ez1-ez0)/0.05 ;
  double gradFromPotential = 0;
  for (int dri = 2; dri < 12; ++dri)
    {
      double runningGrad = 0;
      for (int j = 0; j < size; ++j)
        {
          runningGrad += GetPotGrad4TotalPoint (0, 0, radius + 0.1 * dri, j, dr, config.m_sumNPPotentials, numNPs, 2, pdb, np, potentials, radius, npType);
        }
      // std::cout << radius+0.1*dri << ":" << runningGrad << "\n";
      gradFromPotential = std::max (gradFromPotential, runningGrad);
    }
  // std::cout << "z gradient max from file: " << initialZGradient << "\n";
  // std::cout << "z gradient max from potential" << gradFromPotential/size << "\n";
  initialZGradient = std::min (initialZGradient, std::max (0.0, gradFromPotential / size)); // use the value set in config as a max upper bound

  // std::cout << "estimated total gradient at 1nm above NP: " << gradFromPotential << "\n";
  // std::cout << "suggested pull forcce: " << -gradFromPotential/size << "\n";
  // std::cout << "set to: " << initialZGradient << "\n";
 ...
 */
  ...
  double etaBB = 1e-6; // first mini-update

  std::vector<double> xUpdatesM1 (size, 0.0);
  std::vector<double> yUpdatesM1 (size, 0.0);
  std::vector<double> zUpdatesM1 (size, 0.0);
  std::vector<double> xM1 (size, 0.0);
  std::vector<double> yM1 (size, 0.0);
  std::vector<double> zM1 (size, 0.0);
  ...
  for (int s = 0; s < numSteps; ++s)
    {
      ...
      for (int i = 0; i < size; ++i)
        {
          for (auto &j : nbNeighbours[i])
            { // only loop over neighbours
                  ...
                  if (resDist < wcaSigma * 1.12246)
                    {
                      ...
                      double wcaForceMag = 1.0 * 4.0 * wcaEpsilon * (12 * pow (wcaSigma, 12) / pow (resDist * resDist, 7) - 6 * pow (wcaSigma, 6) / pow (resDist * resDist, 3));
                  ...
                  if (resDist < wcaSigma)
                    {
                      ...
                      double linForceMag = 5000.0;
                      double xForceLJ = dSign (x[i] - x[j]) * linForceMag;
                      double yForceLJ = dSign (y[i] - y[j]) * linForceMag;
                      double zForceLJ = dSign (z[i] - z[j]) * linForceMag;
      ...
      // the initial pulling shifts each bead by a maximum of initialZGradient * numPullSteps  * updateScaleSize
      // uss is c.a. 1e-6 , numPullSteps is total steps/2 ,
      int xFactorPull = 1;
      if (npType == 2 || npType == 4 || npType == 5)
        {
          xFactorPull = 0;
        }
      double surfaceDelta = 1.0; // central-pull force is only switched on when r > radius + surfaceDelta
      for (int i = 0; i < size; ++i)
        {
          ...
          if (s < numPullSteps)
            {
              ...
              // to-do - make it work correctly for cylinders/cubes
              // double centralDist = sqrt( x[i]*x[i] + y[i]*y[i] + z[i]*z[i] ) ;
              double centralDist = getDistance3D (x[i], y[i], z[i], 0.0, 0.0, 0.0, 0.0, npType);
              double surfaceDist = centralDist - radius - surfaceDelta;
              double sigmoidFactor = 1.0 / (1.0 + exp (-1.0 * surfaceDist / 0.1)); // switch off the long-range pulling in the region around the bounding radius

              double currentPullMag = sigmoidFactor * initialZGradient;
              double sinThetaFactor = sqrt (1.0 - z[i] * z[i] / (centralDist * centralDist));
              currentPullMag = currentPullMag; // pulling force is weakest directly above the pole to minimise distortions where the protein is already in contact
              if (s > numPullSteps / 2.0)
                {
                  double sprime = s - numPullSteps / 2.0;
                  double spmax = numPullSteps / 2.0;
                  currentPullMag = currentPullMag - currentPullMag * (sprime / spmax); // gradually reduce the pull over time so it isn't abruptly removed
                }
              ...
              if (doCentralPull == true)
                {
                  ...
                  if (centralDist > 1e-5)
                    {
                      xUpdates[i] -= xFactorPull * currentPullMag * x[i] / centralDist;
                      yUpdates[i] -= currentPullMag * y[i] / centralDist;
                      zUpdates[i] -= currentPullMag * z[i] / centralDist;
                    }
                  // harmonic bond to surface point
                  // if(centralDist > 1e-5){
                  //    xUpdates[i] -= xFactorPull*currentPullMag*x[i]*(radius/centralDist -1.0);
                  //   yUpdates[i] -= currentPullMag*y[i]*(radius/centralDist - 1.0) ;
                  //   zUpdates[i] -= currentPullMag*z[i]*(radius/centralDist - 1.0) ;
                  // }
                }
              else
                {
                  zUpdates[i] -= currentPullMag; // apply the force in the  negative z direction
                }
            }
        }

      bool tuneTimestep = true;

      if (tuneTimestep == true)
        {
          /*
           double maxGrad = 0.0;
           for(int i = 0; i< size; ++i){
               maxGrad = std::max( maxGrad, std::fabs(xUpdates[i]) );
               maxGrad = std::max( maxGrad, std::fabs(yUpdates[i]) );
               maxGrad = std::max( maxGrad, std::fabs(zUpdates[i]) );

           }
          //maxGrad*updateScaleSize < maxDisplacement
          updateScaleSize = 1.0 * maxDisplacement/maxGrad;
          noiseMagnitude = sqrt( 2 * noiseScale  * updateScaleSize*kbTVal );
          //double dtNew = updateScaleSize*kbTVal/nominalDiffusionCoeff  ;
          //std::cout << "timestep set to: " << dtNew << " effective update size " << updateScaleSize << "\n";
         }
          */
          double etaBBNum = 0;
          double etaBBDenom = 1e-12;
          double maxAbsForce = 10.0;
          for (int i = 0; i < size; ++i)
            {
              etaBBNum += (x[i] - xM1[i]) * (xUpdates[i] - xUpdatesM1[i]);
              etaBBNum += (y[i] - yM1[i]) * (yUpdates[i] - yUpdatesM1[i]);
              etaBBNum += (z[i] - zM1[i]) * (zUpdates[i] - zUpdatesM1[i]);
              etaBBDenom += (xUpdates[i] - xUpdatesM1[i]) * (xUpdates[i] - xUpdatesM1[i]);
              etaBBDenom += (yUpdates[i] - yUpdatesM1[i]) * (yUpdates[i] - yUpdatesM1[i]);
              etaBBDenom += (zUpdates[i] - zUpdatesM1[i]) * (zUpdates[i] - zUpdatesM1[i]);

              xM1[i] = x[i];
              yM1[i] = y[i];
              zM1[i] = z[i];
              xUpdatesM1[i] = xUpdates[i];
              yUpdatesM1[i] = yUpdates[i];
              zUpdatesM1[i] = zUpdates[i];
              maxAbsForce = std::max (std::fabs (xUpdates[i]), maxAbsForce);
              maxAbsForce = std::max (std::fabs (yUpdates[i]), maxAbsForce);
              maxAbsForce = std::max (std::fabs (zUpdates[i]), maxAbsForce);
            }
          etaBB = std::fabs (etaBBNum) / etaBBDenom;
          dtNew = etaBB * kbTVal / nominalDiffusionCoeff;

          // std::cout << "suggested scale rate: " << etaBB  << "\n";
          // std::cout << "max-grad scale rate: " << updateScaleSize << "\n";
          // double dtNew = etaBB*kbTVal/nominalDiffusionCoeff  ;
          // std::cout << "equivalent timestep: " << dtNew << "\n";

          if (s > 0)
            {
              updateScaleSize = std::min (etaBB, maxDisplacement / maxAbsForce); // choose whichever is smaller to either enforce stability or optimise the energy
            }
          else
            {
              // updateScaleSize  < maxDisplacement / maxAbsForce
              updateScaleSize = maxDisplacement / maxAbsForce;
            }

          // recompute the noise parameters
          dtNew = updateScaleSize * kbTVal / nominalDiffusionCoeff;
          noiseMagnitude = sqrt (2 * noiseScale * nominalDiffusionCoeff * dtNew);

          // std::cout << "largest displacement will be: " << maxAbsForce * updateScaleSize  << "\n";
          // std::cout << "current noise SDev " << noiseMagnitude << "\n";
          // std::cout << "average per-axis total random displacement over all steps: " << std::sqrt( 2* config.m_relaxSteps * noiseMagnitude << "\n"
        }
      ...
      for (int i = 0; i < size; ++i)
        {
          ...
          // basic method - note that if we're using tunable timesteps, the min-max clipping is already achieved
          /*
          ...
          */
          x[i] += updateScaleSize * xUpdates[i] + xNoiseNP1;
          y[i] += updateScaleSize * yUpdates[i] + yNoiseNP1;
          z[i] += updateScaleSize * zUpdates[i] + zNoiseNP1;
...
int
getMaxThreads (int userMaxThreads, int availableThreads)
{
  if (userMaxThreads < 0)
    {
      std::cout << "Using all available threads (" << availableThreads << ")\n";
      return availableThreads;
    }
  else
    {
      std::cout << "Using " << std::min (userMaxThreads, availableThreads) << "threads\n";
      return std::min (userMaxThreads, availableThreads);
    }
}

void
SaveSpecificOrientation (double phi, double theta, const int size, const Config &config, const Potentials &potentials, const PDB &pdb, const NP &np, double radius, int npType, double cylinderAngle, const std::string &pdbname,
...
// general methodology: find the points contributing to the projected outline
double
getProjectedArea (const int size, double *x, double *y, double *z, double minLoc, double radius, int npType, const Config &config, const PDB &pdb)
{
  // std::cout << "initialising area calc \n";
  double dodecX[20] = { -0.985714, 0.985714, -0.307143, -0.307143, 0.792857, 0.792857, -0.185714, -0.185714, -0.492857, -0.492857, 0.492857, 0.492857, 0.607143, -0.792857, -0.792857, -0.607143, 0.185714, 0.185714, 0.307143, 0.307143 };
  double dodecY[20] = { 0.0, 0.0, -0.935714, 0.935714, -0.578571, 0.578571, -0.578571, 0.578571, -0.357143, 0.357143, -0.357143, 0.357143, 0.0, -0.578571, 0.578571, 0.0, -0.578571, 0.578571, -0.935714, 0.935714 };
  double dodecZ[20] = { 0.185714, -0.185714, 0.185714, 0.185714, 0.185714, 0.185714, 0.792857, 0.792857, -0.792857, -0.792857, 0.792857, 0.792857, -0.792857, -0.185714, -0.185714, 0.792857, -0.792857, -0.792857, -0.185714, -0.185714 };
  double maxArea = 0;

  double projectedArea = 0.0;
  double proteinRadius = 0.0;
  double maxRadius = radius;

  double maxProteinProjectedArea = 0.0;
  double beadRadius = 0.4;
  double maxXExtent = 0.0;
  for (int i = 0; i < size; ++i)
    {
      proteinRadius = std::max (proteinRadius, sqrt (x[i] * x[i] + y[i] * y[i]));
      maxProteinProjectedArea += M_PI * beadRadius * beadRadius;
      maxXExtent = std::max (maxXExtent, std::fabs (x[i]));
    }
  maxXExtent += 0.5; // safety margin
  maxRadius = max (proteinRadius, maxRadius);
  double minRadialRes = 0.1;
  double phiRes = 2 * M_PI / 180; //  std::asin( minRadialRes/maxRadius ) ; //get the phi step size to ensure the perimeter has a resolution of minRadialRes - this is to prevent infinite spikyness
  int numPhiPoints = round (2 * M_PI / phiRes);
  // int beadNumPoints = 8; //number of trial points per bead
  if (npType == 2 || npType == 4 || npType == 5)
    {
      // cylindrical - as a reminder, the cylinder is aligned with the x-axis and is assumed to be infinitely long
      int numAxialPoints = 100;
      double xRes = 2 * maxXExtent / numAxialPoints;
      std::vector<double> phiMinProjectionsAtAxis (numAxialPoints, 0.0);
      std::vector<double> phiMaxProjectionsAtAxis (numAxialPoints, 0.0);
      for (int i = 0; i < size; ++i)
        {
          double zprime = z[i] + minLoc;
          beadRadius = config.AminoAcidRadius (pdb.m_resTag[i]);
          double beadPhi = std::asin (y[i] / sqrt (y[i] * y[i] + zprime * zprime + 1e-6)); // in the yz plane defined relative to the z axis
          double xp = x[i];
          double yp = (radius + beadRadius) * sin (beadPhi);
          double zp = (radius + beadRadius) * cos (beadPhi);
          // int targetXIndex = (int)round( xp/xRes );

          for (int k = 0; k < 20; ++k)
            {
              // generate the dodecahdreal points and project onto the surface coordinate angular basis
              double kx = beadRadius * dodecX[k] + xp;
              double ky = beadRadius * dodecY[k] + yp;
              double kz = beadRadius * dodecZ[k] + zp;
              // double kPointTheta = std::acos( kz/sqrt( kx*kx + ky*ky + kz*kz + 1e-5 ) );
              double kPointPhi = std::asin (ky / sqrt (ky * ky + kz * kz + 1e-6));
              ;                                                                                           // dSign(ky) * std::acos( kx/sqrt( kx*kx + ky*ky + 1e-5 ) ) ;;
              int j = std::min (std::max (0, (int)round ((kx + maxXExtent) / xRes)), numAxialPoints - 1); // safely cast it to the allowed range
              // std::cout << " bead  " <<  i << " point " << k << " mapped to " << kPointPhi << "=" << j << "," <<  kPointTheta <<"\n";
              phiMaxProjectionsAtAxis[j] = std::max (kPointPhi, phiMaxProjectionsAtAxis[j]);
              phiMinProjectionsAtAxis[j] = std::min (kPointPhi, phiMinProjectionsAtAxis[j]);
            }
        }
      for (int j = 0; j < numAxialPoints; ++j)
        { // surface area is r dr dtheta dz
          projectedArea += radius * (phiMaxProjectionsAtAxis[j] - phiMinProjectionsAtAxis[j]) * xRes;
        }
    }
  else if (npType == 3)
    {
      // cubic - same methodology as the sphere, but with beads projected to a face rather than a sphere
      std::vector<double> thetaMinProjectionsAtPhi (numPhiPoints, M_PI);
      std::vector<double> thetaMaxProjectionsAtPhi (numPhiPoints, 0.0);

      for (int i = 0; i < size; ++i)
        {
          double zprime = z[i] + minLoc;
          beadRadius = config.AminoAcidRadius (pdb.m_resTag[i]);
          double beadR0 = sqrt (x[i] * x[i] + y[i] * y[i] + zprime * zprime);
          double beadTheta = std::acos (zprime / (beadR0));

          double beadPhi = dSign (y[i]) * std::acos (x[i] / sqrt (x[i] * x[i] + y[i] * y[i] + 1e-5));

          double maxDPhi = std::asin (beadRadius / (radius + beadRadius));
          // shortest distance from the centre to intersecting with a face
          double rnewX = (radius + beadRadius) / (std::fabs (cos (beadPhi) * sin (beadTheta)) + 1e-5);
          double rnewY = (radius + beadRadius) / (std::fabs (sin (beadPhi) * sin (beadTheta)) + 1e-5);
          double rnewZ = (radius + beadRadius) / (std::fabs (cos (beadTheta)) + 1e-5);
          double rnew = std::min (std::min (rnewX, rnewY), rnewZ);

          double xp = (rnew)*cos (beadPhi) * sin (beadTheta);
          double yp = (rnew)*sin (beadPhi) * sin (beadTheta);
          double zp = (rnew)*cos (beadTheta);
          // std::cout << "projected " << x[i] << " to " << xp   << ", "  << y[i] << " to " << yp << ", "  <<  zprime << " to " << zp   << "\n";
          for (int k = 0; k < 20; ++k)
            {
              // generate the dodecahdreal points and project onto the surface coordinate angular basis
              double kx = beadRadius * dodecX[k] + xp;
              double ky = beadRadius * dodecY[k] + yp;
              double kz = beadRadius * dodecZ[k] + zp;
              // don't bother remapping these again - most of them should already be on one face and the rest can help to round corners
              double kPointTheta = std::acos (kz / sqrt (kx * kx + ky * ky + kz * kz + 1e-5));
              double kPointPhi = dSign (ky) * std::acos (kx / sqrt (kx * kx + ky * ky + 1e-5));
              ;
              int j = std::max (std::min ((int)round ((kPointPhi + M_PI) / phiRes), numPhiPoints), 0);
              // std::cout << " bead  " <<  i << " point " << k << " mapped to " << kPointPhi << "=" << j << "," <<  kPointTheta <<"\n";
              thetaMaxProjectionsAtPhi[j] = std::max (kPointTheta, thetaMaxProjectionsAtPhi[j]);
              thetaMinProjectionsAtPhi[j] = std::min (kPointTheta, thetaMinProjectionsAtPhi[j]);
            }
        }
      // now loop over all phi segments and add the contribution to the area
      // double maxArea = 0;
      for (int j = 0; j < numPhiPoints; ++j)
        {
          // std::cout << j << "/" << numPhiPoints <<  "\n";
          thetaMinProjectionsAtPhi[j] = std::min (thetaMinProjectionsAtPhi[j], thetaMaxProjectionsAtPhi[j]);
          // double areaTerm = ( -1.0* std::cos(thetaMaxProjectionsAtPhi[j])  + std::cos(thetaMinProjectionsAtPhi[j]) )  *radius*radius*phiRes;
          // std::cout << j << " " << ( j*phiRes*180/M_PI ) <<  " adding segment from " << thetaMinProjectionsAtPhi[j] << " to " << thetaMaxProjectionsAtPhi[j] << " area: " << areaTer>
          // instead of summing along constant radius, now we integrate along the path of a ] segment
          double areaTerm = 0;
          double thetaUpperSegment = 0.785398; // 0 < theta < theta_upper bounds the upper plane
          // theta_upper < theta < theta_low bounds the side plane
          double thetaLowerSegment = 2.35619; // theta_low < theta < pi bounds the lower plane

          double upperPlaneStart = std::min (thetaMinProjectionsAtPhi[j], thetaUpperSegment);
          double upperPlaneEnd = std::min (thetaMaxProjectionsAtPhi[j], thetaUpperSegment);
          double upperPlaneTerm = radius * radius / cos (upperPlaneEnd) - radius * radius / cos (upperPlaneStart);

          double middlePlaneStart = std::min (std::max (thetaMinProjectionsAtPhi[j], thetaUpperSegment), thetaLowerSegment);
          double middlePlaneEnd = std::min (thetaMaxProjectionsAtPhi[j], thetaLowerSegment);
          middlePlaneEnd = std::max (middlePlaneStart, middlePlaneEnd);
          double middlePlaneTerm = radius * radius * (log (tan (middlePlaneEnd)) - log (tan (middlePlaneStart)));

          double lowerPlaneStart = std::min (std::max (thetaMinProjectionsAtPhi[j], thetaLowerSegment), M_PI);
          double lowerPlaneEnd = std::min (M_PI, thetaMaxProjectionsAtPhi[j]);
          lowerPlaneEnd = std::max (lowerPlaneStart, lowerPlaneEnd);
          double lowerPlaneTerm = radius * radius / cos (lowerPlaneEnd) - radius * radius / cos (lowerPlaneStart);
          // std::cout << "theta spans: " << thetaMinProjectionsAtPhi[j] << " to " << thetaMaxProjectionsAtPhi[j] << "\n";
          // std::cout << "upper term spans: " << upperPlaneStart << " to " << upperPlaneEnd << " produces: " << upperPlaneTerm <<"\n";
          // std::cout << "middle term spans: " << middlePlaneStart << " to " << middlePlaneEnd << " produces: " << middlePlaneTerm <<"\n";
          // std::cout << "lower term spans: " << lowerPlaneStart << " to " << lowerPlaneEnd << " produces: " << lowerPlaneTerm <<"\n";

          areaTerm = phiRes * (lowerPlaneTerm + middlePlaneTerm + upperPlaneTerm);

          projectedArea += areaTerm;
          // std::cout << "updated area \n";
          // maxArea += 2*radius*radius*phiRes;
        }
      // std::cout << "final area: " << projectedArea << "\n";
    }
  else
    {
      std::vector<double> thetaMinProjectionsAtPhi (numPhiPoints, M_PI);
      ...
      for (int i = 0; i < size; ++i)
        {
          ...
          double maxDPhi = std::asin (beadRadius / (radius + beadRadius));
          double xp = (radius + beadRadius) * cos (beadPhi) * sin (beadTheta);
          double yp = (radius + beadRadius) * sin (beadPhi) * sin (beadTheta);
          double zp = (radius + beadRadius) * cos (beadTheta);

          // std::cout << dSign(y[i]) << "\n";
          // std::cout << "bead loc: " <<  x[i] << "," << y[i] << "," << zprime <<  "\n";
          // std::cout << "bead projected loc: " <<  xp << "," << yp << "," << zp <<  "\n";

          // std::cout << "bead phi: " << beadPhi << " bead theta " << beadTheta << "\n";

          // spherical basis points
      ...
      for (int j = 0; j < numPhiPoints; ++j)
        {
          ...
          thetaMinProjectionsAtPhi[j] = std::min (thetaMinProjectionsAtPhi[j], thetaMaxProjectionsAtPhi[j]);
          double areaTerm = (-1.0 * std::cos (thetaMaxProjectionsAtPhi[j]) + std::cos (thetaMinProjectionsAtPhi[j])) * radius * radius * phiRes;
          // std::cout << j << " " << ( j*phiRes*180/M_PI ) <<  " adding segment from " << thetaMinProjectionsAtPhi[j] << " to " << thetaMaxProjectionsAtPhi[j] << " area: " << areaTerm << "\n";
          projectedArea += areaTerm;
          // std::cout << "updated area \n";
          // maxArea += 2*radius*radius*phiRes;
        }
      // std::cout << "completed area calc \n";
    }
  // std::cout << "final area: " << projectedArea  << "\n";
  // std::cout << " planar limit spread area: " << maxProteinProjectedArea << "\n";
  // std::cout << "maximum area: " << maxArea << " max theory area: " << radius*radius*4*M_PI << "\n";
  return projectedArea;
}

void
AdsorptionEnergies (const PDB &pdb, const NP &np, const Config &config, const Potentials &potentials, const double radius, const double outerRadius, const int angle_offset, const int n_angles, double *adsorption_energy, double *adsorption_error,
                    double *mfpt_val, double *mfpt_err, double *minloc_val, double *minloc_err, double *numcontacts_val, double *numcontacts_err, double *protein_offset_val, double *protein_offset_err, double *protein_area_val,
                    double *protein_area_err, const std::string &pdbname, const std::string &outputdirectory, const std::string &npName, int npType = 1, double imaginary_radius = -1, int calculateMFPT = 0, double cylinderAngleDeg = 0,
                    double zeta = 0, int savePotentials = 0, double temperature = 300.0, double overlapPenalty = 0.0)
{
  ...
  double sample_proteinArea[samples];
  ...
  // }
  // std::cout << " starting thread for " << phi << "," << theta << "\n" <<  std::flush;
  ...
  for (int angle = 0; angle < n_angles; ++angle)
    {
      ...
      for (int sample = 0; sample < samples + 1; ++sample)
        {
                  ...
                  for (k = 0; k < numNPBeadSummation; ++k)
                    {
                      ...
                      if (distance < 0.5)
                        { // count close-range interactions for later summary statistics
                          resHasContacted = 1;
                        }
                      ...
                      */
                      ...
                      /*
                                   if(config.m_potNoiseMag > 100){
                                    //noiseEnergy +=     unitNormalDist(randomEngine)*  (  std::min( std::fabs(noiseEnergy*0.1) , config.m_potNoiseMag)); //gaussian noise with zero mean and magnitude of 10% of the energy or the value specified in the
                         config file, whichever is smaller
              ...
              if (savePotentials == 1)
                {
                  ...
                  // std::cout<<phi << " " << theta << "\n";
                  filename = outputdirectory + "/" + npName + "/" + pdbname + "_" + std::to_string (static_cast<int> (radius)) + "_" + std::to_string (static_cast<int> (1000 * zeta)) + "_" + std::to_string (static_cast<int> (phi * 180 / M_PI)) + "_"
                             + std::to_string (static_cast<int> (theta * 180 / M_PI)) + cylinderFileNameAppend + ".uap";
                  std::ofstream handle (filename.c_str ());
                  handle << "#rcc,E(kbT)\n";
                  for (i = 0; i < steps; ++i)
                    {
          ...
          // getProjectedArea (const int size, double *x, double *y, double *z, double minLoc,  double radius, int npType)
          double proteinArea = getProjectedArea (size, x, y, z, minLoc, radius, npType, config, pdb);
          ...
          sample_proteinArea[sample] = proteinArea;
          /*   for (i = 0; i < steps; ++i) {
      ...
      MeanAndSD (samples, &(protein_area_val[angle_offset + angle]), &(protein_area_err[angle_offset + angle]), sample_proteinArea);
    }
}

void
SurfaceScan (const PDB &pdb, const Potentials &potentials, const double zeta, const double radius, const double outerRadius, const Config &config, double omegaAngle, const NP &np)
{
  ...
  double protein_area_val[iterations] = {};
  double protein_area_err[iterations] = {};
  ...
  if (checkPrecalc == true)
    ...
    bool fileHasProximity = false;
     if(updateContactMapFile == true){
      ...
      double phiMin = 0.0;
      double thetaMin = 0.0;
      double ccdAtMin = radius + 50.0;
      double energyMin = 500.0;
      std::fstream handle(precalcFileName.c_str());
      if(handle.is_open() ){
        while (std::getline(handle, line)) {
            ...
            if(  line.substr(0, 1) != "#"){
              ...
              if(trialEnergy  < energyMin){
                  energyMin = trialEnergy;
                  phiMin = std::stod( line.substr( 0,7 ) );
                  thetaMin = std::stod( line.substr( 7,7 ) );
                  ccdAtMin = std::stod( line.substr( 119,14 ) );
                  //std::cout << " updated minima: " << phiMin << " " << thetaMin << " " << energyMin << " " << ccdAtMin << " " << "\n";
              }
            }
        }
     handle.close();
    ...
    {
#ifdef PARALLEL
      const int n_threads = getMaxThreads (config.m_maxThreads, omp_get_max_threads ()); //  omp_get_max_threads();
#else
...
#pragma omp for schedule(dynamic, 1)
        ...
        for (thread = 0; thread < n_threads; ++thread)
          {
            AdsorptionEnergies (pdb, np, config, potentials, radius, outerRadius, thread * n_per_thread + (thread < n_remaining ? thread : n_remaining), n_per_thread + (thread < n_remaining), adsorption_energy, adsorption_error, mfpt_val, mfpt_err,
                                minloc_val, minloc_err, numcontacts_val, numcontacts_err, protein_offset_val, protein_offset_err, protein_area_val, protein_area_err,

                                pdb.m_name, config.m_outputDirectory, np.m_name, config.m_npType, imaginary_radius, config.m_calculateMFPT, omegaAngle, zeta, config.m_savePotentials, config.m_temperature, config.m_overlapPenalty
...
main (const int argc, const char *argv[])
{
  ...
  PDBs pdbs (targetList.m_paths, config.AminoAcidIdMap (), ligandMap.m_ligandAALookup, config.m_backboneReplaceSet, config.m_disorderStrat, config.m_disorderMinBound, config.m_disorderMaxBound, config.m_readLigands, config.m_bondCutoffNM,
             config.m_enableBackbone, config.m_backboneTag, config.m_backboneScale);
          ...
          for (const double zetaPotential : config.m_zetaPotential)
            {
              ...
              std::string npTypeOutString = "TYPE," + to_string (nanoparticleRadius) + "," + to_string (zetaPotential) + "," + "1,1," + to_string (config.m_npType) + "," + config.m_hamakerFile + "," + config.m_pmfDirectory + ","
                                            + to_string (config.m_PMFCutoff) + "," + to_string (config.m_npType) + "\n";
